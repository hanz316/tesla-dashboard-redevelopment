...
- **输入只有两件事**：左右滑动换页、点击设置项。滑动阈值 120 px，且必须
  以水平为主（竖直行程更大的手势不算换页——车在动时误触不能翻页）。
  滑动手势"内容跟着手指"：向左拖 = 下一页。
- **设置项可操作**：点一下设置行就步进该值（亮度 ±10% 并钳制在
  10–100%，永不归零；默认页只在可到达的 8 页里循环，永远不会变成
  Developer）。行命中框在 `input_router_v6` 里，与场景标签位置由测试逐条
  比对（`tests/v6_binding_coverage_tests.py`）。
